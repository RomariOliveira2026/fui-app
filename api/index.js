...
function cumulativePathDistances(path3) {
  if (path3.length === 0) return [];
  const cum = [0];
  for (let i = 1; i < path3.length; i++) {
    cum.push(cum[i - 1] + haversineMeters(path3[i - 1], path3[i]));
  }
  return cum;
}
function pathTotalMeters(path3) {
  const cum = cumulativePathDistances(path3);
  return cum[cum.length - 1] ?? 0;
}
function densifyPath(path3, maxStepMeters = DEFAULT_STEP_METERS) {
  if (path3.length < 2) return path3.slice();
  const out = [path3[0]];
  for (let i = 1; i < path3.length; i++) {
    const a = path3[i - 1];
    const b = path3[i];
...
function projectPointOnPath(path3, point) {
  if (path3.length === 0) {
    return { point, meters: 0, progress: 0 };
  }
  if (path3.length === 1) {
    return { point: path3[0], meters: 0, progress: 0 };
  }
  const cum = cumulativePathDistances(path3);
  const total = cum[cum.length - 1] ?? 0;
  let bestDist = Infinity;
  let bestPoint = path3[0];
  let bestMeters = 0;
  for (let i = 1; i < path3.length; i++) {
    const a = path3[i - 1];
    const b = path3[i];
...
function pointAtPathMeters(path3, meters) {
  const total = pathTotalMeters(path3);
  if (path3.length === 0) return { lat: 0, lng: 0 };
  if (total <= 0) return path3[0];
  return pointAtPathProgress(path3, Math.max(0, Math.min(1, meters / total)));
}
function pointAtPathProgress(path3, progress) {
  if (path3.length === 0) return { lat: 0, lng: 0 };
  if (path3.length === 1 || progress <= 0) return path3[0];
  if (progress >= 1) return path3[path3.length - 1];
  const total = pathTotalMeters(path3);
  if (total <= 0) return path3[0];
...
function trimPathFromMeters(path3, fromMeters) {
  if (path3.length < 2 || fromMeters <= 0) return path3;
  const cum = cumulativePathDistances(path3);
  const start = pointAtPathMeters(path3, fromMeters);
  for (let i = 1; i < path3.length; i++) {
    if (cum[i] >= fromMeters) {
      const tail = path3.slice(i);
...
function cacheDemoRoutePath(rideId, path3) {
  if (path3.length >= 2) {
    routeCache.set(rideId, densifyPath(path3));
  }
}
...
function buildTrack(ride, phase) {
  ...
  const path3 = buildDriverPhasePath(tripPath, phase, {
  ...
  const distanceM = Math.max(pathTotalMeters(path3), 120);
...
function buildSegment(ride, phase, target) {
  ...
  const distanceM = Math.max(pathTotalMeters(path3), 100);
  const durationMs = Math.max(
    SEGMENT_DURATION_MS,
    Math.round(distanceM / 1e3 / 36 * 3600 * 1e3)
  );
  return { path: path3, target, startedAtMs: Date.now(), durationMs };
}
...
// server/vercel.ts
var app = createApp({ enableStatic: false });
